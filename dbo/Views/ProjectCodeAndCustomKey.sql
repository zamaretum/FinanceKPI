...
/* =========
   Parameters
   ========= */
DECLARE @person_key        VARCHAR(20) = '3896';
DECLARE @years_back        INT        = 3;   -- main lookback (years)
DECLARE @extra_years       INT        = 1;   -- additional years used in all_fiscal_accounts seed
DECLARE @today_date        DATE       = CAST(GETDATE() AS DATE);
DECLARE @begin_cutoff_4yr  DATE       = DATEADD(YEAR, -(@years_back + @extra_years), @today_date);
DECLARE @begin_cutoff_3yr  DATE       = DATEADD(YEAR, -@years_back, @today_date);
...
;WITH all_fiscal_accounts AS (
    SELECT
        ...
        CAST(0 AS DECIMAL(38, 10))               AS amount,
        a.type                                   AS acc_type,
    ...
    WHERE fm.begin_date >= @begin_cutoff_4yr
),
re_accounts_cte AS (
    SELECT
        gl.account_key                                  AS account_key,
        ...
        SUM(CASE WHEN a.type = 'E'
                 THEN (gl.debit_amount - gl.credit_amount)
                 WHEN a.type = 'R'
                 THEN (gl.credit_amount - gl.debit_amount)
                 ELSE 0 END)                            AS amount,
        SUM(CASE WHEN a.type = 'E'
                 THEN (gl.local_debit_amount - gl.local_credit_amount)
                 WHEN a.type = 'R'
                 THEN (gl.local_credit_amount - gl.local_debit_amount)
    ...
    SELECT
        ...
        afa.end_date,
        CAST(0 AS DECIMAL(38,10))                       AS amount,
        CAST(0 AS DECIMAL(38,10))                       AS local_amount,
...
totals AS (
    /* A/L running balances seeded from GL plus gaps from all_fiscal_accounts */
    SELECT
    ...
    /* E/R cumulative for next period (with reset at period 1) */
    SELECT
        x.account_key,
        x.org_key,
        ...
        fm_next.fiscal_month_key                 AS fiscal_month_key,
        x.fiscal_month_key                       AS prev_fiscal_month_key,
        CASE WHEN fm_next.period_number = 1 THEN CAST(0 AS DECIMAL(38,10)) ELSE SUM(y.amount) END        AS amount,
        SUM(y.amount)                                                                                   AS amount_c,
        CASE WHEN fm_next.period_number = 1 THEN CAST(0 AS DECIMAL(38,10)) ELSE SUM(y.local_amount) END AS local_amount,
...
/* Used by wrE6 (project attributes) */
...
SELECT
    ...
    wrE19."Project Key"     AS "Project Key"
FROM (
    /* =========================
       wrE19 (GL + balances view)
       ========================= */
    SELECT
    ...
    FROM (
        ...
        SELECT
            ...
            CASE a.type WHEN 'A' THEN 'Asset'
                       WHEN 'E' THEN 'Expense'
                       WHEN 'L' THEN 'Liability'
                       WHEN 'R' THEN 'Revenue'
                       ELSE 'Unknown Account Type' END AS "Acct Type",
            CASE a.type WHEN 'A' THEN 1
                       WHEN 'L' THEN 2
                       WHEN 'R' THEN 3
                       WHEN 'E' THEN 4 END            AS "Acct Type Order",
            ...
            CASE WHEN fm_prev.period_number < 10
                 ...
                 ELSE CONCAT(CONCAT(fy.name,'-'), CAST(fm_prev.period_number AS VARCHAR(4))) END
                                                      AS "Fiscal Period",
            NULL                                     AS "Post Date",
            NULL                                     AS "Quantity",
        ...
        WHERE NOT (t_p.amount = 0 AND t.amount = 0)
          AND fm.begin_date >= @begin_cutoff_3yr
        ...
        SELECT
            ...
            CASE WHEN fm.period_number < 10
                 ...
                 ELSE CONCAT(CONCAT(fy.name,'-'), CAST(fm.period_number AS VARCHAR(4))) END
                                                    AS "Fiscal Period",
        ...
        LEFT JOIN [dbo].["aretum"."customer"] le   ON le.customer_key      = org.legal_entity_key
        LEFT JOIN [dbo].["aretum"."customer"] cust ON cust.customer_key    = gl.customer_key
        ...
        LEFT JOIN [dbo].["aretum"."project"]  proj ON proj.project_key     = gl.project_key
        LEFT JOIN [dbo].["aretum"."customer"] porg ON porg.customer_key    = proj.customer_key
        LEFT JOIN [dbo].["aretum"."customer"] pown ON pown.customer_key    = proj.owning_customer_key
        LEFT JOIN [dbo].["aretum"."person"]   pers ON pers.person_key      = gl.person_key
        ...
        WHERE fm.begin_date >= @begin_cutoff_3yr
    ) x
    WHERE
...
INNER JOIN (
    ...
    /* Consolidation trees (via top-level CTE consol_tree_orgs) */
...
LEFT OUTER JOIN (
    ...
    SELECT
        ...
        CASE proj.bill_rate_source WHEN 'P' THEN 'P - Person Bill Rate'
                                   WHEN 'L' THEN 'L - Labor Category Bill Rate' END
                                                   AS "Proj Bill Rate Source",
        bt.code                                    AS "Proj Billing Type Code",
        CASE WHEN EXISTS (SELECT 1 FROM [dbo].["aretum"."member"] WHERE person_key = @person_key AND role_key IN (1,21,37,39))
             THEN proj.Exp_Bill_Budget ELSE NULL END  AS "Proj Budget Exp Bill Amt",
        CASE WHEN EXISTS (SELECT 1 FROM [dbo].["aretum"."member"] WHERE person_key = @person_key AND role_key IN (1,21,38,40))
        ...
        CASE proj.cost_rate_source WHEN 'P' THEN 'P - Person Cost Rate'
                                   WHEN 'L' THEN 'L - Labor Category Cost Rate' END
                                                   AS "Proj Cost Rate Source",
        ...
        CASE WHEN ce.code IS NOT NULL THEN CONCAT(CONCAT(cs.cost_structure, '-'), ce.code) END AS "Default Labor Cost Element",
    ...
    LEFT JOIN [dbo].["aretum"."contract"]          mcn  ON mcn.contract_key = cn.prime_contract_key
    ...
    LEFT JOIN [dbo].["aretum"."project_fee"]       pf   ON pf.project_key            = proj.project_key
