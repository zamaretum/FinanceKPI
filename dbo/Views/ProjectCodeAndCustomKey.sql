...
FROM (
    ...
    totals as (
        select
            ...
            sum(amount) over( partition by x.account_key, x.org_key order by x.begin_date )                       amount,
            sum(amount) over( partition by x.account_key, x.org_key order by x.begin_date )                       amount_c,
            sum(local_amount) over( partition by x.account_key, x.org_key order by x.begin_date )                 local_amount,
            sum(local_amount) over( partition by x.account_key, x.org_key order by x.begin_date )                 local_amount_c,
        ...
        join fiscal_month fm_next
            on fm_next.begin_date = x.end_date + INTERVAL '1' day
...
INNER JOIN (
    ...
    left outer join consol_tree_orgs le_consol_tree_orgs
    ...
    left outer join consol_tree_orgs org_consol_tree_orgs
...
LEFT OUTER JOIN (
    ...
    select
        ...
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,37,39)) then proj.Exp_Bill_Budget
            else null
        end                                     as "Proj Budget Exp Bill Amt",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,38,40)) then proj.exp_cost_burden_budget
            else null
        end                                     as "Proj Budget Exp Burdened Cost",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,38,40)) then proj.exp_cost_budget
            else null
        end                                     as "Proj Budget Exp Cost Amt",
        proj.hours_budget                       as "Proj Budget Hours",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,37,39)) then proj.labor_bill_budget
            else null
        end                                     as "Proj Budget Lab Bill Amt",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,38,40)) then proj.labor_cost_burden_budget
            else null
        end                                     as "Proj Budget Lab Burd Cost",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,38,40)) then proj.Labor_Cost_Budget
            else null
        end                                     as "Proj Budget Lab Cost Amt",
        ...
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,37,39)) then proj.exp_bill_est_tot
            else null
        end                                     as "Proj Est Tot Exp Bill Amt",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,38,40)) then proj.exp_cost_est_tot
            else null
        end                                     as "Proj Est Tot Exp Cost Amt",
        proj.hours_est_tot                      as "Proj Est Total",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,37,39)) then proj.labor_bill_est_tot
            else null
        end                                     as "Proj Est Tot Lab Bill Amt",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,38,40)) then proj.labor_cost_est_tot
            else null
        end                                     as "Proj Est Tot Lab Cost Amt",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,37,39)) then proj.Exp_Bill_Etc
            else null
        end                                     as "Proj ETC Exp Bill Amt",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,38,40)) then proj.exp_cost_etc
            else null
        end                                     as "Proj ETC Exp Cost Amt",
        proj.hours_etc                          as "Proj ETC Hours",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,37,39)) then proj.labor_bill_etc
            else null
        end                                     as "Proj ETC Lab Bill Amt",
        case
            when exists(select person_key from member where person_key = '3896' and role_key in (1,21,38,40)) then proj.labor_cost_etc
            else null
        end                                     as "Proj ETC Labor Cost Amount",
        ...
        case
            when i_s.project_key is null then 'N'
            else 'Y'
        end                                     as "Intercompany Effort",
        i_s.pi_list								as "Intercompany Support",
        case
            when ce.code is not null then concat(concat(cs.cost_structure,'-'),ce.code )
            else null
        end   									as "Default Labor Cost Element",
